 ...
 */
class ProductDetailAdapterImages : ListAdapter<ProductDetail.Images, ProductDetailAdapterImages.ProductViewHolder>(
    POST_COMPARATOR
) {

    inner class ProductViewHolder(private val binding: ItemBannerBinding) :
        RecyclerView.ViewHolder(binding.root) {
        fun bind(item: ProductDetail.Images) {
            binding.apply {
                ivBanner.loadAny(item.image?:""){
                    crossfade(true)
                    error(R.drawable.tour_backpacker_mandalika)
                }
...
//                }
    ...
    companion object {
        val POST_COMPARATOR = object : DiffUtil.ItemCallback<ProductDetail.Images>() {
            override fun areContentsTheSame(
                oldItem: ProductDetail.Images,
                newItem: ProductDetail.Images
